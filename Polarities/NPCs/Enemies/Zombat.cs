...
namespace Polarities.NPCs.Enemies
{
    public class Zombat : ModNPC
    {
        ...
        {
            EntitySource_SpawnNPC source = new();
        ...
        {
            return Terraria.ModLoader.Utilities.SpawnCondition.OverworldNightMonster.Chance * 0.05f;
        }

        public override bool CheckDead()
        {
            for (int i = 1; i <= 3; i++)
                Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, Mod.Find<ModGore>("ZombatGore" + i).Type);
            return true;
        }

        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            npcLoot.Add(ItemDropRule.Common(ItemType<Items.Weapons.Summon.Minions.ZombatStaff>(), 10));
