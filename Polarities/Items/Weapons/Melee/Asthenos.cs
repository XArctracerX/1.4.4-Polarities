...
namespace Polarities.Items.Weapons.Melee
{
    ...
    {
        public static PreGeneratedRandom asthenosRandomValues = new(51801, 4095);
        ...
        {
            Projectile.damage = (int)(Projectile.damage * Projectile.ai[0] / MAX_CHARGE);
            hitDirection = Main.player[Projectile.owner].direction;

            Player player = Main.player[Projectile.owner];
            if (!player.channel)
            {
                Projectile.damage = (int)(Projectile.damage * Projectile.ai[0] / 10);
            }
        }
